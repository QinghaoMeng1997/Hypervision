...
def generate_and_compute():
    ...
    inputs.tofile("test/tlin/in.bin")
    ...
    weights = np.random.rand(BATCH_SIZE, head, pre_c, feature) # 生成浮点数 0~1
    weights = weights*2 - 1 #-1~1
    weights = weights.astype(np.float32)# float64 to float32
    weights.tofile("test/tlin/w.bin")
    with open(weight_path, 'w') as file:
        for n in range(BATCH_SIZE):
            for h in range(head):
                for f in range(pre_c):
                    for s in range(feature):
